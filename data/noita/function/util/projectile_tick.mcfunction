...
$summon marker ~ ~ ~ {Tags: ["noita.projectile", "noita.new", "noita.proj.tick1", "noita.proj.tick", $(features)], data: {on_hit: "$(on_hit)", on_tick: "$(on_tick)", particle: "$(particle)", speed: $(speed), display: false, damage_type: "$(damage_type)", tick_rate: $(tick_rate)}}
